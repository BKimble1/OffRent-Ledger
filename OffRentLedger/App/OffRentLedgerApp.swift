...
struct OffRentLedgerApp: App {
    ...
    var body: some Scene {
        WindowGroup {
            ...
            // The app's appearance, set in exactly one place.
            //
            // It used to be set twice — `.light` here and the user's setting inside `RootView` —
            // and the outer one wins, so Settings › Appearance was a picker that did nothing.
            // Here it covers the launch splash and the store-recovery screen as well as the app,
            // which the one inside `RootView` never could.
            //
            // The default is light rather than "match iOS": the visual layer was designed
            // against warm white and graphite and that is the one to meet first. Dark is a
            // choice somebody can make, and it is measured — `check_text_colours_meet_contrast`
            // runs every text token against all three surfaces in both modes.
